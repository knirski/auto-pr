...
import type { CommitInfo } from "../../scripts/fill-pr-template-core.js";
...
function runWithLogAndFilesEffect(
	...
	}).pipe(Effect.provide(TestBaseLayer), Effect.provide(FillPrTemplateLiveLayer));
}
...
describe("fillTemplate", () => {
	...
	test("howToTestDefault overrides default for non-docs-only", () => {
		...
		const data = fillTemplate(commits, ["src/foo.ts"], undefined, "1. Run `pytest`\n2. ");
		expect(data.howToTest).toBe("1. Run `pytest`\n2. ");
	});
});
...
const RunFillBodyTestLayer = Layer.mergeAll(TestBaseLayer, FillPrTemplateLiveLayer);
