...
/** Create repo with commits that touch files. Each entry: { message, files: [{ path, content }] }. */
...
layer(TestLayer)("runAutoPrGetCommits", (it) => {
	...
	it.effect("writes files.txt with changed file paths when commits touch files", () =>
		Effect.gen(function* () {
