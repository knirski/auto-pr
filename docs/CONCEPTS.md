...
| Term | Meaning |
|------|---------|
| **FC/IS** | Functional Core / Imperative Shell. The core (`src/auto-pr/core.ts`, `src/lib/*.ts`) contains pure functions returning `Result`; no Effect, no I/O. The shell (`src/auto-pr/shell.ts`) orchestrates I/O and bridges via `Effect.fromResult`. |
