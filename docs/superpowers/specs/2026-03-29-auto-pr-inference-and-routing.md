...
**Summary:** **Which** backend (`llamacpp` vs `github-models`), **`AUTO_PR_AI_*`**, **pre-generate model id** (no router inside `generate-content`), **metrics → band → allowlisted model**, and **prompt** placeholders. Single stack: `LanguageModel` via `@effect/ai-openai-compat`. **Ollama removed** — [migration](2026-03-29-ollama-to-llamacpp-migration-design.md).

## Independence from tools

**Baseline needs no `Tool`/`Toolkit`:** metrics, model selection, and routing context can ship with **`generateObject`** + offline git + commit text. Tools are an **optional** upgrade for targeted diffs/files ([toolkit spec](2026-03-29-auto-pr-effect-toolkit-design.md)). **Band → model id** stays **allowlisted in code**, never LLM-chosen.
...
Only **`AUTO_PR_AI_PROVIDER` ∈ { `llamacpp`, `github-models` }**. Both use `OpenAiClient.layer` + `OpenAiLanguageModel.model` + `FetchHttpClient`. No separate `openai-compat` enum — remote OpenAI-compatible hosts use **`llamacpp`** with URL + model + optional key.
...
**Reality:** Perfect “importance” ranking without human review or deep analysis is **not** available. Pre-inference steps can only **denoise, bucket, and order** input (denylists, `delta_src` vs `delta_raw`, `gen_ratio`, heuristics on paths/subjects). The model (optionally with **tools**) does **synthesis** and emphasis. Pattern: **curate budget** → **summarize**; see **§3** for metrics.
...
**Why separate step:** Quota/cost policy (e.g. GitHub Models free tier: small PR → mini; hard PR → stronger id).
...
Cover **typical** PRs and **edge** cases: huge diffs (source vs `dist/`), generated-heavy PRs, synthesis-hard multi-area work. Use **two dimensions:** (1) **volume** — meaningful change to summarize; (2) **hardness** — difficulty unifying commits. Avoid using **`delta_raw`** alone when lockfiles dominate.
...
**Always** validate against the live [GitHub Models catalog](https://github.com/marketplace/models) or API. Names/tiers change.
...
**Don’t:** feed unbounded diff just because metrics are large — still truncate/path-filter (§1 “importance”, §3.4).
