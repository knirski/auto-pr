...
**`bun run test:integration`** runs those tests with `--no-coverage` ([`bunfig.integration.toml`](../bunfig.integration.toml); coverage is tracked on the unit job only). Local llama scenarios use Testcontainers with the image pin in `.github/llama-server/Dockerfile` (**Docker** required). Set **`INTEGRATION_SKIP_DOCKER=1`** to skip Docker-based tests. The **GitHub Models** integration test needs **`GH_TOKEN`** with **`models: read`** in your environment when running locally (for example export a PAT). In Actions, that real cloud-provider scenario runs only on manual/nightly integration runs.
