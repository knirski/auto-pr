...
**`bun run test:integration`** runs those tests with `--no-coverage` ([`bunfig.integration.toml`](../bunfig.integration.toml); coverage is tracked on the unit job only). Local llama scenarios use Testcontainers with the image pin in `.github/llama-server/Dockerfile` when run directly (**Docker** required). In GitHub Actions, the workflow starts llama-server first and the tests reuse `AUTO_PR_AI_OPENAI_COMPAT_URL` / `AUTO_PR_AI_OPENAI_COMPAT_MODEL` instead of starting a second container. Set **`INTEGRATION_SKIP_DOCKER=1`** to skip Docker-based tests. The **GitHub Models** integration test needs **`GH_TOKEN`** with **`models: read`** in your environment when running locally (for example export a PAT). In Actions, that real cloud-provider scenario runs only on manual/nightly integration runs.
...
**auto-pr.yml** runs on push to `ai/**` branches (including forks). Two reusable workflows: generate (unprivileged checkout + content) and create (trusted checkout + PR). The generate job uses composite actions from this repo; adopters do not vendor shell under `scripts/`. The model routing context action runs a checked-in Node bundle from its own action directory, so non-JavaScript adopter repositories do not need Bun or auto-pr dependencies. Security model: [docs/WORKFLOW_SECURITY.md](WORKFLOW_SECURITY.md). Forks need `APP_ID` and `APP_PRIVATE_KEY` in their repo secrets to succeed. See [docs/INTEGRATION.md](INTEGRATION.md).
...
**update-bun-nix.yml** runs on manual trigger (workflow_dispatch). Use when `main` has a stale `bun.nix` (e.g. after merging a lockfile change from a fork or Dependabot PR). Runs on the default branch and pushes the updated `bun.nix` to `main`. For trusted same-repo PRs, the **`nix`** job in `ci.yml` handles updates automatically when paths match.
...
## Action-local generated bundles

`.github/actions/auto-pr-build-model-routing-context/auto-pr-build-model-routing-context.mjs` is generated by `bun run build` from TypeScript/Effect source in the same action directory. It is committed with the action because remote composite actions run from `GITHUB_ACTION_PATH` inside adopter workflow runs, while `GITHUB_WORKSPACE` is the adopter repository.

This bundle is intentionally different from root `dist/`:

- **Action-local bundle:** Runtime artifact for a reusable composite action. Commit it whenever the action source changes.
- **Root `dist/`:** Package runtime output for `npx` / Node-only installs. Do not commit it in ordinary PRs; update workflows own it.

The composite action must run with `node`, not `bun`, because adopter repositories can be Python, Rust, Go, or npm projects that never install Bun. If the action source or bundle changes, workflow self-reference pins still need to point at one commit that contains every referenced workflow and action path.
