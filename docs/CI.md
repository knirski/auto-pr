...
| PR to main (code changes) | `ci.yml`: `check` → [check.yml](../.github/workflows/check.yml) + local-model `integration` → [integration.yml](../.github/workflows/integration.yml) + `dependency-review` when paths match `code` |
| PR to main (nix only) | `ci.yml`: `nix` → [nix.yml](../.github/workflows/nix.yml) when paths match `**/*.nix` or `flake.lock` |
...
| Manual | update-bun-nix, update-flake-lock, update-workflow-pins, update-dist, integration, SBOM |
| Scheduled | integration real GitHub Models smoke + SBOM (nightly), update-flake-lock (Sun), scorecard (Sat), stale (Mon) |
...
2. **Codecov** (optional) — Add `CODECOV_TOKEN` for the coverage badge and test analytics. The **`check`** job uploads unit coverage (`coverage/lcov.info`) and JUnit (`test-report.junit.xml`). The **`integration`** job does not upload to Codecov. Get the token from [codecov.io](https://codecov.io). Without it, upload steps no-op; CI still passes. To approximate Codecov patch coverage locally, run `bun test` and then `bun run check:patch-coverage [base-ref]` (default base: `main`). The local check compares `git diff <base>...HEAD` changed `src/**/*.ts` lines with `coverage/lcov.info`; Codecov's server-side result is authoritative, but this catches common uncovered-patch failures before push.
...
4. **Branch protection** — Require **`CI / gate`** (see [Branch Protection](#branch-protection)). Required PR integration covers deterministic local-model scenarios; real GitHub Models and SBOM generation run outside the PR gate on nightly/manual workflows.

### Integration job and fork PRs

The required PR **integration** job runs local llama scenarios only. The real **GitHub Models** smoke test uses `GITHUB_TOKEN` with `models: read`, so it runs on `workflow_dispatch` and nightly `schedule` instead of blocking dependency and fork PRs.
...
**`bun run test:integration`** runs those tests with `--no-coverage` ([`bunfig.integration.toml`](../bunfig.integration.toml); coverage is tracked on the unit job only). Local llama scenarios use Testcontainers with the image pin in `.github/llama-server/Dockerfile` when run directly (**Docker** required). In GitHub Actions, the workflow starts llama-server first and the tests reuse `AUTO_PR_AI_OPENAI_COMPAT_URL` / `AUTO_PR_LOCAL_MODEL` instead of starting a second container. Set **`INTEGRATION_SKIP_DOCKER=1`** to skip Docker-based tests. The **GitHub Models** integration test needs **`GH_TOKEN`** with **`models: read`** in your environment when running locally (for example export a PAT). In Actions, that real cloud-provider scenario runs only on manual/nightly integration runs.
...
| [sbom.yml](../.github/workflows/sbom.yml) | workflow_dispatch, schedule | — | Generate and upload CycloneDX SBOM |
...
**ci.yml** is the single entry for push/PR to `main`: the `changes` job applies path filters so each downstream job runs only when relevant. Pure Nix file changes (`**/*.nix`, `flake.lock`) now fan out only to the dedicated **`nix`** job, while Bun/app dependency changes still go through **`check`** and **`integration`**. The **`gate`** job aggregates outcomes for branch protection as **`CI / gate`**. The **`nix`** job calls [nix.yml](../.github/workflows/nix.yml): upstream Nix ([cachix/install-nix-action](https://github.com/cachix/install-nix-action)), statix/deadnix via `nix flake check`. `nix.yml` splits check from mutation ([ADR 0006](adr/0006-nix-ci-upstream-and-caching.md)'s security note, [ADR 0016](adr/0016-immutable-privileged-workflow-executor.md)): every `pull_request` (same-repo or fork) only runs the read-only `bun-nix-check`/`build` jobs, which regenerate `bun.nix` in-memory and fail with a local-remediation message if it's stale — they never push. Only `push:[main]` and the manual `update-bun-nix.yml` dispatch reach the privileged `bun-nix-push` job (the `app-credentials` environment, same GitHub App as auto-pr), which commits and pushes `bun.nix` using an App token so the push triggers a new CI run (GITHUB_TOKEN pushes do not trigger workflows). Dependabot PRs are check-only because GitHub does not expose repository secrets to Dependabot-triggered runs.
...
**update-bun-nix.yml** runs on manual trigger (workflow_dispatch) only — it has no `pull_request` or `push` trigger. Use when `main` has a stale `bun.nix` (e.g. after merging a lockfile change from a fork or Dependabot PR, or any PR whose stale-`bun.nix` check failed and wasn't fixed locally before merge). Runs on the default branch and pushes the updated `bun.nix` to `main`. No PR — same-repo or fork — gets `bun.nix` auto-fixed-and-pushed by CI; see `nix.yml`'s header comment and [`test/scripts/nix-credential-boundary.test.ts`](../test/scripts/nix-credential-boundary.test.ts).
...
**GitHub Models job:** The **`integration-github-models`** job is skipped in required `ci.yml` PR/push calls and runs only for `workflow_dispatch` or nightly `schedule`. It loads **`.env.ci`** with **`omit_llama_integration_env: true`** so llama-related keys and **`AUTO_PR_AI_OPENAI_COMPAT_URL`** from **`.env.ci`** are not applied (that job uses GitHub Models only).
...
## Stale `bun.nix` in a PR

No PR — same-repo or fork — gets `bun.nix` auto-fixed-and-pushed by CI (see the **ci.yml** entry under [Workflows](#workflows) above, and [nix.yml](../.github/workflows/nix.yml)'s header comment). If the **`nix`** job's `bun-nix-check` fails with "bun.nix is out of date", update locally: `nix run .#update-bun-nix`, then commit and push. See [CONTRIBUTING.md](../CONTRIBUTING.md).
