...
**auto-pr.yml** runs on push to `ai/**` branches (including forks). Two reusable workflows: generate (unprivileged checkout + content) and create (trusted checkout + PR). The generate job uses reusable actions from this repo; adopters do not vendor shell under `scripts/`. The model routing context action runs as a Node 24 JavaScript action from its own checked-in bundle, so non-JavaScript adopter repositories do not need Bun or auto-pr dependencies. Security model: [docs/WORKFLOW_SECURITY.md](WORKFLOW_SECURITY.md). Forks need `APP_ID` and `APP_PRIVATE_KEY` in their repo secrets to succeed. See [docs/INTEGRATION.md](INTEGRATION.md).
...
`.github/actions/auto-pr-build-model-routing-context/auto-pr-build-model-routing-context.mjs` is generated by `bun run build` from TypeScript/Effect source in the same action directory. It is committed with the action because remote reusable actions run from `GITHUB_ACTION_PATH` inside adopter workflow runs, while `GITHUB_WORKSPACE` is the adopter repository.

This bundle is intentionally different from root `dist/`:

- **Action-local bundle:** Runtime artifact for a reusable action. Commit it whenever the action source changes.
- **Root `dist/`:** Package runtime output for `npx` / Node-only installs. Do not commit it in ordinary PRs; update workflows own it.

The model routing context action runs as a JavaScript action with `runs.using: node24`, not through `bun`, because adopter repositories can be Python, Rust, Go, or npm projects that never install Bun. If the action source or bundle changes, workflow self-reference pins still need to point at one commit that contains every referenced workflow and action path.
