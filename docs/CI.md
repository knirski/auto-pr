...
| [act-smoke.yml](../.github/workflows/act-smoke.yml) | push, pull_request → main, workflow_dispatch | `paths: .github/workflows/**`, `scripts/run-check-act.ts`, `flake.nix` | `--dry-run check-workflows`, `--dry-run check`, then `check-workflows` via [`gh-act`](https://github.com/nektos/gh-act) |
