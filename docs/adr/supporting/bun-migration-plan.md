...
- [scripts/act-local-ci.ts](../../../scripts/act-local-ci.ts): optional local CI parity (`bun run act`); runs `check` then `integration`; see [CONTRIBUTING](../../../CONTRIBUTING.md#run-ci-locally-check-job) for `gh act`, nektos `act`, and Nix-on-Linux (`nix run .#act`)
