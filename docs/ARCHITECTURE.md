...
2. **generate-content** — Parse commits → 1 commit: fill from body; 2+: LanguageModel (AI provider) via `generateObject` → fill template → title, body_file
...
- **AI integration:** `src/auto-pr/live/ai-provider.ts` (provider dispatcher), `src/auto-pr/live/ollama-language-model.ts` (Ollama adapter); `src/workflow/auto-pr-generate-content.ts` calls `LanguageModel.generateObject` for PR title/description
