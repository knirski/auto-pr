...
- **Local CI parity (optional):** `scripts/run-check-act.ts` (`bun run act`) — Docker + nektos act or `gh act`; pure planning in `src/core/act-local-ci.ts`. See [CONTRIBUTING.md](../CONTRIBUTING.md#run-ci-locally-full-parity).
