...
**When changing `src/`:** Run `bun run build` before tests if needed. `dist/` is gitignored; the `prepare` script builds it when installing from git (e.g. `bun add github:knirski/auto-pr` or `npx -p github:knirski/auto-pr`).
