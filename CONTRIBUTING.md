...
**Fork contributors:** Fork the repo, clone your fork, then `npm install` and `npx lefthook install`. Push to `ai/*` branches to auto-create PRs. The auto-PR workflow runs on forks; it will fail with "Missing secrets" unless you add `APP_ID` and `APP_PRIVATE_KEY` to your fork's **Settings → Secrets and variables → Actions** (create a GitHub App for your fork). Without secrets, create the PR manually from your branch to `main`.
...
Lefthook runs `npm run check:code` before each push. Run `npx lefthook install` after cloning to enable hooks. Uses only npm deps (audit, test, lint, knip, typecheck); no typos/lychee/actionlint required. Skip with `git push --no-verify` if needed.
