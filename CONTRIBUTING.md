...
**check:just-links** and **check:with-links** can fail on broken external URLs (404s, redirects, timeouts). Use `bun run check:just-links` to verify links locally. In CI, the reusable `check.yml` and `check-docs.yml` workflows fail the job if lychee reports errors. Lychee accepts 200 and 429 (rate limit) via `--accept 200,429`.
