...
- `scripts/auto-pr/core.ts` — Pure helpers (filterSemanticSubjects, formatGhOutput, etc.)
- `scripts/fill-pr-template-core.ts` — Pure PR template logic (parseCommits, fillTemplate, renderBody)
- `scripts/auto-pr/config.ts` — Config services (Schema-validated env)
- `scripts/auto-pr/shell.ts` — runCommand, appendGhOutput, runMain
- `scripts/auto-pr/interfaces/` — FillPrTemplate interface
- `scripts/auto-pr/live/` — FillPrTemplate live interpreter
- `scripts/*.ts` — CLI entry points (auto-pr-get-commits, generate-pr-content, create-or-update-pr, fill-pr-template)
